...
struct TemplatesView: View {
        ...
        VStack(alignment: .leading, spacing: Theme.Spacing.md) {
            HStack {
                Image(systemName: category.icon)
                    .foregroundStyle(Color(adaptiveHex: category.color))
    ...
    private func categoryFilterButton(category: TemplateCategory?, label: String) -> some View {
        Button(action: {
            ...
            .padding(.vertical, 8)
            .background(backgroundColorForButton(category))
            .foregroundStyle(foregroundColorForButton(category))
            .cornerRadius(20)
            .overlay(
                RoundedRectangle(cornerRadius: 20)
                    .stroke(strokeColorForButton(category), lineWidth: 1)
            )
        }
    }

    // Helper methods for category filter button colors
    private func backgroundColorForButton(_ category: TemplateCategory?) -> Color {
        if selectedCategory == category {
            if let cat = category {
                return Color(adaptiveHex: cat.color).opacity(0.15)
            } else {
    ...
    private func foregroundColorForButton(_ category: TemplateCategory?) -> Color {
        if selectedCategory == category {
            if let cat = category {
                return Color(adaptiveHex: cat.color)
            } else {
                return Color.blue
            }
        } else {
    ...
    private func strokeColorForButton(_ category: TemplateCategory?) -> Color {
        if selectedCategory == category {
            if let cat = category {
                return Color(adaptiveHex: cat.color).opacity(0.5)
            } else {
